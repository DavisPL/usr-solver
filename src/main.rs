...
use classes::{GenRegex, Predicate, CharExpression,  StringVar, StringIndex};
use print::{print_predicate, print_equals_arg, print_char_expression,  print_string_var, print_gre};
use predicateEvaluation::{evaluateComplete, convertToDNF, flatten_and_predicates};
use std::rc::Rc;
use either::Either;

fn derivative(gre: &Rc<GenRegex>, deriv_char: &Rc<CharExpression>) ->Rc<GenRegex>{
    let empty_string = || Rc::new(GenRegex::CharExpression(
        Rc::new(CharExpression::Literal(String::new()))
    ));
    ...
    match gre.as_ref() {
        ...
        GenRegex::CharExpression(cExpr)=>{
            return simplifies(&Rc::new(GenRegex::IfThenElse(Rc::new(Predicate::Equals(Either::Left(Rc::clone(cExpr)), Either::Left(Rc::clone(deriv_char)))), empty_string(), empty_set())));
        }
        GenRegex::StringVar(sVar)=>{
                return simplifies(&Rc::new(GenRegex::IfThenElse(Rc::new(Predicate::Equals(Either::Right(Rc::new(StringIndex{var: Rc::clone(sVar), index: 0})), Either::Left(Rc::clone(deriv_char)))), Rc::new(GenRegex::StringSlice(Rc::clone(sVar), 1)), empty_set())));

        }
        GenRegex::StringSlice(stringVar, index)=> {
                return simplifies(&Rc::new(GenRegex::IfThenElse(Rc::new(Predicate::Equals(Either::Right(Rc::new(StringIndex{var: Rc::clone(stringVar), index: *index})), Either::Left(Rc::clone(deriv_char)))), Rc::new(GenRegex::StringSlice(Rc::clone(stringVar), index+1)), empty_set())));
        }
        GenRegex::Union(side1, side2) =>{
            return simplifies(&Rc::new(GenRegex::Union(Rc::clone(&derivative(&Rc::clone(side1), deriv_char)), Rc::clone(&derivative(&Rc::clone(side2), deriv_char)))));
        }
        GenRegex::Intersect(side1, side2) =>{
            return simplifies(&Rc::new(GenRegex::Intersect(Rc::clone(&derivative(&Rc::clone(side1), deriv_char)), Rc::clone(&derivative(&Rc::clone(side2), deriv_char)))));
        }
        GenRegex::Concatenation(side1, side2) =>{
            let leftSide = Rc::new(GenRegex::Concatenation(Rc::clone(&derivative(&Rc::clone(side1), deriv_char)), Rc::clone(side2)));
            let rightSide = Rc::new(GenRegex::Concatenation(Rc::clone(&nullable(&Rc::clone(side1))), Rc::clone(&derivative(&Rc::clone(side2), deriv_char))));

            return simplifies(&Rc::new(GenRegex::Union(Rc::clone(&leftSide), Rc::clone(&rightSide))));
        }
        GenRegex::Complement(side1) =>{
            return simplifies(&Rc::new(GenRegex::Complement(Rc::clone(&derivative(&Rc::clone(side1), deriv_char)))));
        }
        GenRegex::Kleene(side1) =>{
            return simplifies(&Rc::new(GenRegex::Concatenation(Rc::clone(&derivative(&Rc::clone(side1), deriv_char)), Rc::clone(side1))));
        }
        GenRegex::IfThenElse(pred, side1, side2) =>{
            return simplifies(&Rc::new(GenRegex::IfThenElse(Rc::clone(pred), Rc::clone(&derivative(&Rc::clone(side1), deriv_char)), Rc::clone(&derivative(&Rc::clone(side2), deriv_char)))));
        }
        GenRegex::StringIndex(string_index) =>{
            return simplifies(&Rc::new(GenRegex::IfThenElse(Rc::new(Predicate::Equals(Either::Right(Rc::clone(&string_index)), Either::Left(Rc::clone(deriv_char)))), empty_string(),empty_set() )));
        }
    }
...
fn nullable(gre: &Rc<GenRegex>) -> Rc<GenRegex>{
    match gre.as_ref() {
        ...
        GenRegex::StringSlice(string_var, index) =>{
                return Rc::new(GenRegex::IfThenElse(Rc::new(Predicate::EqualLength(Rc::clone(string_var), *index)), Rc::new(GenRegex::CharExpression(Rc::new(CharExpression::Literal(String::from(""))))), Rc::new(GenRegex::EmptySet)))

            }
            GenRegex::StringVar(string_var) => {
                return Rc::new(GenRegex::IfThenElse(Rc::new(Predicate::EqualLength(Rc::clone(string_var), 0)), Rc::new(GenRegex::CharExpression(Rc::new(CharExpression::Literal(String::from(""))))), Rc::new(GenRegex::EmptySet)))
        ...
        GenRegex::Union(side1, side2) =>{
            return Rc::new(GenRegex::Union(Rc::clone(&nullable(&Rc::clone(side1))), Rc::clone(&nullable(&Rc::clone(side2)))))
        }
        GenRegex::Intersect(side1, side2) =>{
            return Rc::new(GenRegex::Intersect(Rc::clone(&nullable(&Rc::clone(side1))), Rc::clone(&nullable(&Rc::clone(side2)))))
        }
        GenRegex::Concatenation(side1, side2) =>{
            return Rc::new(GenRegex::Concatenation(Rc::clone(&nullable(&Rc::clone(side1))), Rc::clone(&nullable(&Rc::clone(side2)))))
        }
        GenRegex::Complement(side1) =>{
            return Rc::new(GenRegex::Intersect(Rc::new(GenRegex::Complement(Rc::clone(&nullable(&Rc::clone(side1))))), Rc::new(GenRegex::CharExpression(Rc::new(CharExpression::Literal(String::from("")))))))
        }
        GenRegex::Kleene(side1) =>{
            return Rc::new(GenRegex::CharExpression(Rc::new(CharExpression::Literal(String::from("")))))
        }
        GenRegex::IfThenElse(pred, side1, side2) =>{
            return Rc::new(GenRegex::IfThenElse(Rc::clone(pred), Rc::clone(&nullable(&Rc::clone(side1))), Rc::clone(&nullable(&Rc::clone(side2)))))
        }
    }
    ...
    match expr.as_ref() {
        ...
        GenRegex::IfThenElse(pred, true_expr, false_expr) => {
            ...
            match (true_proj.as_ref(), false_proj.as_ref()) {
                ...
                (_, Predicate::False) => Rc::new(Predicate::And(vec![
                    Rc::clone(pred),
                    Rc::clone(&true_proj)]
                )),
...
fn simplifies(gre: &Rc<GenRegex>) -> Rc<GenRegex> {
    match gre.as_ref() {
        ...
        GenRegex::Concatenation(left_side, right_side) => simplify_concatenation(left_side, right_side),
        GenRegex::IfThenElse(pred, true_branch, false_branch) => simplify_if_then_else(pred, true_branch, false_branch),
        _ => Rc::clone(gre),
    }
}
...
fn simplify_intersect(left_side: &Rc<GenRegex>, right_side: &Rc<GenRegex>) -> Rc<GenRegex> {
    ...
    match (&*left, &*right) {
        ...
        (GenRegex::IfThenElse(pred1, true1, false1), GenRegex::IfThenElse(pred2, true2, false2)) => {
...
fn simplify_intersect_if_then_else(
    ...
    match (false1.as_ref(), false2.as_ref()) {
        ...
        (_, GenRegex::EmptySet) => create_complex_if_then_else(pred1, pred2, true1, true2, false1, false2, true),
        (GenRegex::EmptySet, _) => create_complex_if_then_else(pred2, pred1, true2, true1, false2, false1, false),
...
fn create_complex_if_then_else(
    ...
    let inner_pred = if is_first_branch {
        Rc::new(Predicate::And(vec![Rc::clone(pred2), Rc::new(Predicate::Not(Rc::clone(pred1)))]))
    } else {
        Rc::new(Predicate::And(vec![Rc::clone(pred1), Rc::new(Predicate::Not(Rc::clone(pred2)))]))
    };
...
fn simplify_if_then_else(
    ...
    match (&*simplified_true, &*simplified_false) {
        (GenRegex::EmptySet, GenRegex::EmptySet) => {return simplified_true;},
        _ => {}
    };
    ...
    match pred.as_ref() {
        Predicate::Equals(left, right) => {
            if let (Either::Left(c1), Either::Left(c2)) = (left.as_ref(), right.as_ref()) {
                if let (CharExpression::Literal(val1), CharExpression::Literal(val2)) = (c1.as_ref(), c2.as_ref()) {
                    return if val1 == val2 {
                        simplified_true
                    } else {
                        simplified_false
    ...
    Rc::new(GenRegex::IfThenElse(Rc::clone(pred), simplified_true, simplified_false))
}
...
fn main() {
    ...
    let predicate = Predicate::Equals(
        ...
        Either::Right(Rc::new(StringIndex { var: Rc::clone(&string_var), index: 0 })),
    );
    let intersect = &Rc::new(GenRegex::Intersect(Rc::new(GenRegex::StringVar(Rc::clone(&string_var))), Rc::new(GenRegex::CharExpression(Rc::new(CharExpression::Literal(String::from("a")))))));
    ...
    let gre2 = &Rc::new(GenRegex::Concatenation(Rc::clone(stringVarGre), Rc::clone(stringVarGre)));
    let gre3 = &Rc::new(GenRegex::Concatenation(Rc::clone(gre2), Rc::new(GenRegex::CharExpression(Rc::new(CharExpression::Literal(String::from("a")))))));
