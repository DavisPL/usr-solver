...
def evaluate2(predicate, unionFind=None):
    ...
    if isinstance(predicate, AndPred):
        ...
        for p in Equalities:
            try:
                if unionFind.union(p.left, p.right) == EmptySet():
                    return EmptySet()
                else:
                    FinalPreds.append(p)
            except:
        ...
        for p in NotEqualityPreds:
            ...
            if isinstance(q, Equals):
                ...
                for key, value in unionFind.parent.items():
                    ...
                    if isinstance(tempKey, StringIndex) and tempKey.stringVar == p.left.name and tempKey.index >= p.right:
                ...
                #pObj = unionFind.stringToObject(unionFind.find(StringIndex(q.left, q.right)))
                    ...
                    #FinalPreds.append(p)
                #if isinstance
        for p in LengthPreds:
            if p.right in NotAllowedLengths.get(p.left.name, set()):
                return EmptySet()
        ...
        final_set = []
        for p in predicate.predicates:
            p_eval = evaluate(p, UnionFind())
            if isinstance(p_eval, Literal) and p_eval.value == "":
                return Literal("")
            elif not isinstance(p_eval, EmptySet):
                final_set.append(p_eval)
                
        if not final_set:
            return EmptySet()
        if len(final_set) == 1:
            return final_set[0]
        return UnionPred(*final_set)
    ...
    if isinstance(predicate, AndPred):
        ...
        for p in all_predicates:
            if isinstance(p, Not):
                ...
                temp = LengthPreds.get(unionFind.simplify(p.left.name), None)
...
def convertToDNF2(predicate):
    if isinstance(predicate, Equals):
        return predicate
    elif isinstance(predicate, UnionPred):
        return UnionPred(*[convertToDNF(child) for child in predicate.predicates])
    elif isinstance(predicate, AndPred):
        #print(predicate.predicates)
        return distributeOrs([convertToDNF(child) for child in predicate.predicates])
    elif isinstance(predicate, Not):
        if isinstance(predicate.predicate, AndPred):
            return UnionPred(*[convertToDNF(Not(child)) for child in predicate.predicate.predicates])
        elif isinstance(predicate.predicate, UnionPred):
...
def distributeOrs2(predicates):
    ...
    dnf_result = [AndPred(*group) for group in product]
...
def distributionAnd(clauses):
    ...
    # Check if any of the predicates are OrPreds
        ...
        if isinstance(predicate, OrPred):
            ...
            for child in predicate.predicates:
                for existing in result:
                    new_result.append(AndPred(existing, child))
            result = new_result  # Update result with new combinations
        else:
            result.append(predicate)  # Add the AND predicate directly

    # Return the overall expression as an OR of the result clauses
    return OrPred(*result)  # Combine all clauses with OR
